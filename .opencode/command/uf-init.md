...
### Step 9: AGENTS.md User Profile Guidance

If the repository has an `AGENTS.md` file, ensure it
contains the "User Profile" guidance section that
instructs all agents to read
`.uf/onboarding/profile.md` when it exists.

1. **Read** `AGENTS.md` in the repository root
2. **Check** if a `## User Profile` heading exists
   (search for the heading text `## User Profile`)
3. **If already present**: Report
   `--- AGENTS.md: User Profile section already present (skipped)`
4. **If not present**: Find the `## Testing Conventions`
   heading and insert the following block BEFORE it.
   If `## Testing Conventions` is not found, append the
   block at the end of the file. Report
   `+++ AGENTS.md: User Profile section injected`
5. **If AGENTS.md does not exist**: Report
   `--- AGENTS.md: not found (skipped)` and continue
