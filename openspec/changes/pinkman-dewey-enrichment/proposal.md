...
Snoopy stores scouting results in Dewey via
`dewey_store_learning` using a flat `snoopy` tag and
...
only surface Snoopy's findings when the query happens
to semantically overlap with the terse summary text.

Enriching the tag taxonomy and content structure of
Snoopy's Dewey learnings improves the probability that
...
Modify Snoopy's `dewey_store_learning` instructions in
the agent file to use:

1. **Mode-specific tags** -- `snoopy-discover`,
   `snoopy-trend`, `snoopy-audit`, `snoopy-report`
   instead of flat `snoopy`. Hyphen-separated because
...
### New Capabilities
- `mode-specific-tag-taxonomy`: Snoopy learnings use
  mode-specific tags under the `snoopy-` prefix
  (`snoopy-discover`, `snoopy-audit`, etc.), enabling
...
- `dewey-learning-storage`: The existing Dewey
  integration section in snoopy.md is updated with
...
- **No hero agent modifications**: Cobalt-Crush,
  ...
  queries will surface enriched Snoopy learnings through
...
- **Backward compatible**: The `dewey_store_learning` API
  ...
  mode-specific tags (`snoopy-discover`, etc.) are valid
  free-form strings per Spec 021 conventions. Old
  learnings stored with flat `snoopy` remain discoverable
...
Snoopy is a non-hero utility agent. The inter-hero
...
Heroes discover Snoopy's findings through their existing
...
Snoopy is not installed, no hero is affected. If Dewey
is unavailable, Snoopy still writes Markdown reports to
`.uf/snoopy/reports/` (graceful degradation already
...
Snoopy's existing Markdown reports with YAML frontmatter
